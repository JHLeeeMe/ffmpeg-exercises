...
int main()
{
    ...
    while (av_read_frame(in_fmt_ctx, in_packet) == 0)
    {
        ...
        {
            int ret = avcodec_send_packet(v_decoder_ctx, in_packet);
            if (ret < 0)
            {
            ...
            while (true)
            {
                ret = avcodec_receive_frame(v_decoder_ctx, in_frame);
                if (ret < 0)
                {
                    ...
                    {
                        avcodec_flush_buffers(v_decoder_ctx);
                        break;
                    }
                    else
                    {
        ...
        {
            int ret = avcodec_send_packet(a_decoder_ctx, in_packet);
            if (ret < 0)
            {
                av_packet_unref(in_packet);
                continue;
            ...
            while (true)
            {
                ret = avcodec_receive_frame(a_decoder_ctx, in_frame);
                if (ret < 0)
                {
                    if (ret == AVERROR(EAGAIN))
                    {
                    ...
                    {
                        avcodec_flush_buffers(a_decoder_ctx);
                        break;
                    }
                    else
                    {
